...
class ActivityFeed:
    ...
    def subscribe(self, sub_key: str) -> asyncio.Queue:
        ...
        logger.info(f"ActivityFeed: new subscriber for {sub_key}, total={len(self.subscribers[sub_key])}")
        return queue
    ...
    def unsubscribe(self, sub_key: str, queue: asyncio.Queue):
        ...
        try:
            ...
            logger.info(f"ActivityFeed: subscriber removed for {sub_key}, total={len(self.subscribers[sub_key_key])}")
        except ValueError:
            pass
    ...
    async def publish(self, sub_key: str, data: dict):
        ...
        logger.info(f"ActivityFeed: published to {len(queues)} subscriber(s) for {sub_key}")
